...
export default function setPlayerAction(player: Player) {
    ...
    touchController.subscribe('START', (touch) => {
        ...
        touch.subscribe('MOVE', (lastPoint) => {
            const pickInfo = player.world.scene.pick(
                lastPoint.x,
                lastPoint.y,
                (mesh) => mesh === player.world.skyboxMesh || mesh === player.world.groundMesh,
            );
            if (pickInfo.hit) {
                brick.linearVelocity = BABYLON.Vector3.Zero();

                brick.mesh.position = pickInfo.pickedPoint.scale((1 / pickInfo.pickedPoint.length()) * 10);
        ...
        touch.subscribe('END', (lastPoint) => {
            ...
            if (pickInfo.hit) {
                brick.linearVelocity = pickInfo.pickedPoint
                    ...
                    .add(new BABYLON.Vector3(0, 30, 0));
            } else {
                //todo
            }
